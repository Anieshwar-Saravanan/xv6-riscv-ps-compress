...
user/_testactive:     file format elf64-littleriscv
...
main(void)
{
  ...
  int count = getactiveprocesses(pids, MAX_PROCESSES);
   8:	04000593          	li	a1,64
   c:	ed040513          	addi	a0,s0,-304
  10:	3ba000ef          	jal	3ca <getactiveprocesses>
  
  if (count < 0) {
  14:	04054063          	bltz	a0,54 <main+0x54>
  18:	ee26                	sd	s1,280(sp)
  1a:	ea4a                	sd	s2,272(sp)
  1c:	e64e                	sd	s3,264(sp)
  1e:	892a                	mv	s2,a0
  ...
  24:	8e850513          	addi	a0,a0,-1816 # 908 <malloc+0x11c>
  28:	70c000ef          	jal	734 <printf>
  for (int i = 0; i < count; i++) {
  2c:	03205163          	blez	s2,4e <main+0x4e>
  30:	ed040493          	addi	s1,s0,-304
  34:	090a                	slli	s2,s2,0x2
  36:	9926                	add	s2,s2,s1
    printf("%d\n", pids[i]);
  38:	00001997          	auipc	s3,0x1
  3c:	8e898993          	addi	s3,s3,-1816 # 920 <malloc+0x134>
  40:	408c                	lw	a1,0(s1)
  42:	854e                	mv	a0,s3
  44:	6f0000ef          	jal	734 <printf>
  for (int i = 0; i < count; i++) {
  48:	0491                	addi	s1,s1,4
  4a:	ff249be3          	bne	s1,s2,40 <main+0x40>
  }

  exit(0);
  4e:	4501                	li	a0,0
  50:	2ba000ef          	jal	30a <exit>
  54:	ee26                	sd	s1,280(sp)
  56:	ea4a                	sd	s2,272(sp)
  58:	e64e                	sd	s3,264(sp)
    printf("Error retrieving active processes\n");
  5a:	00001517          	auipc	a0,0x1
  5e:	88650513          	addi	a0,a0,-1914 # 8e0 <malloc+0xf4>
  62:	6d2000ef          	jal	734 <printf>
    exit(1);
  66:	4505                	li	a0,1
  68:	2a2000ef          	jal	30a <exit>
...
stat(const char *n, struct stat *st)
{
 ...
 1e6:	84aa                	mv	s1,a0
  ...
  r = fstat(fd, st);
 1e8:	85ca                	mv	a1,s2
 1ea:	178000ef          	jal	362 <fstat>
 1ee:	892a                	mv	s2,a0
  close(fd);
 1f0:	8526                	mv	a0,s1
 1f2:	140000ef          	jal	332 <close>
  return r;
 1f6:	64a2                	ld	s1,8(sp)
}
 1f8:	854a                	mv	a0,s2
...
00000000000003ca <getactiveprocesses>:
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 414:	52080813          	addi	a6,a6,1312 # 930 <digits>
 418:	88be                	mv	a7,a5
...
vprintf(int fd, const char *fmt, va_list ap)
{
 ...
 696:	29eb8b93          	addi	s7,s7,670 # 930 <digits>
 ...
 6e0:	24c48493          	addi	s1,s1,588 # 928 <malloc+0x13c>
        for(; *s; s++)
